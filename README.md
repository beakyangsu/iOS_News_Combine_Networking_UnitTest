...
+ ### Why Combine for Networking?
  ...
  + 사실 Networking에 있어서 중요한점은 비동기(aync)로 동작해야한다는 점인데 사실 Combine은 비동기 프로그래밍이 아닌 State변화를 관찰 및 Update하기 위한 Framework지만 변화가있을시 Update라는 특징이 비동기프로그래밍과도 비슷한 부분이 있어 Combine을 사용하는 경우도 많다고 하여 사용해봤습니다.
...
+ ### UnitTest
  ...
  + 이럴경우 코드를 수정한 뒤 UnitTest 실행을 통해 <ins>기존에 정상동작이던 기능들이 문제없이 동작하는지 빠르게 확인</ins>할 수 있습니다.
  + **안정적인 기능구현과, 유지보수**하기 쉽게 만들어줍니다.
